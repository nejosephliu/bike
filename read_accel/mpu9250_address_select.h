...
void mpu9250_init(const nrf_twi_mngr_t* i2c, uint8_t device_address);

// Read all three axes on the accelerometer
//
// Return measurements as floating point values in g's
mpu9250_measurement_t mpu9250_read_accelerometer(uint8_t device_address);

// Read all three axes on the gyro
//
// Return measurements as floating point values in degrees/second
mpu9250_measurement_t mpu9250_read_gyro(uint8_t device_address);
